...
# Try to modify /etc/hosts, but don't fail if we can't (restricted environment)
echo "Attempting to update /etc/hosts..."
if echo "127.0.0.1 $ORG_NAME.docs.buildwithfern.com.localhost" >> /etc/hosts 2>/dev/null; then
    echo "::1 $ORG_NAME.docs.buildwithfern.com.localhost" >> /etc/hosts
    echo "Successfully updated /etc/hosts for $ORG_NAME.docs.buildwithfern.com.localhost"
else
    echo "WARNING: Cannot modify /etc/hosts (likely due to restricted permissions), continuing without hosts file modification"
fi

# If CUSTOM_DOMAIN is set and not null, set CUSTOM_DOMAIN_CLEANED for later use
if [ -n "$CUSTOM_DOMAIN" ] && [ "$CUSTOM_DOMAIN" != "null" ]; then
    CUSTOM_DOMAIN_CLEANED=$(echo "$CUSTOM_DOMAIN" | sed -E 's#^https?://##' | cut -d'/' -f1 | tr -d ':')
    if echo "127.0.0.1 $CUSTOM_DOMAIN_CLEANED.localhost" >> /etc/hosts 2>/dev/null; then
        echo "::1 $CUSTOM_DOMAIN_CLEANED.localhost" >> /etc/hosts
        echo "Successfully updated /etc/hosts for $CUSTOM_DOMAIN_CLEANED.localhost"
    else
        echo "WARNING: Cannot modify /etc/hosts for custom domain (likely due to restricted permissions), continuing without hosts file modification"
    fi
else
