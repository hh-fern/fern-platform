import {
  ...
  resolveLocalPreviewBundleTarPath,
  zipLocalBundle,
} from "../src/docs-fe-stack";
...
async function mainZipLocalBundle() {
  ...
  try {
    const localPreviewBundleTarPath =
      resolveLocalPreviewBundleTarPath(zipFilePathArg);
    await zipLocalBundle(localPreviewBundleTarPath);
    console.log(
      `Successfully zipped ${LOCAL_PREVIEW_BUNDLE_OUT_DIR} to ${localPreviewBundleTarPath}`
    );
  } catch (err) {
    console.error(
      "Error zipping local bundle:",
