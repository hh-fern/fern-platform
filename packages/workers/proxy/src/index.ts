...
export default {
    async fetch(request, _env, _ctx): Promise<Response> {
        ...
        if (request.headers.get("Upgrade") === "websocket") {
            if (request.method !== "GET" || !["wss:"].includes(forwardedUrl.protocol)) {
                return new Response(null, { status: 400 });
            }
            return upgradeToWebsocket(forwardedUrl);
        }

        const allowedRequestHeaders = (request.headers.get(REQUEST_HEADERS) ?? "").toLowerCase().split(",");
        ...
        for (const [key, value] of request.headers) {
            if (allowedRequestHeaders.length === 0 || allowedRequestHeaders.includes(key.toLowerCase())) {
        ...
        if (
            ...
            response.headers.get("Content-Type")?.toLowerCase().startsWith("application/octet-stream")
        ) {
