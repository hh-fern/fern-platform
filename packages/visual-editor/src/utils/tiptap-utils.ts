/** biome-ignore-all lint/suspicious/noConsole: we want to console.warn and console.error */
...
export function focusNextNode(editor: Editor) {
    ...
    if (!paragraphType) {
        console.warn("No paragraph node type found in schema."); // eslint-disable-line no-console
        return false;
    }
...
export function isExtensionAvailable(editor: Editor | null, extensionNames: string | string[]): boolean {
    ...
    if (!found) {
        console.warn(
            `None of the extensions [${names.join(", ")}] were found in the editor schema. Ensure they are included in the editor configuration.` // eslint-disable-line no-console
        );
    }
...
const ATTR_WHITESPACE =
    ...
    // biome-ignore lint/suspicious/noControlCharactersInRegex: we want to allow control characters
    /[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g;
