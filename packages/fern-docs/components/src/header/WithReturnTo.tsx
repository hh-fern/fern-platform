...
export function WithReturnTo({
    ...
    React.useEffect(() => {
        try {
            ...
            url.searchParams.set(queryParam, String(new URL(pathname, window.location.origin)));
            setHref(String(url));
        } catch (error) {
            console.error(`Failed to set return to query param ${queryParam} for ${pathname}`, error);
