...
import { isMdxExpression, isMdxJsxAttribute, isMdxJsxElementHast } from "../mdx-utils";
import type { Hast, Mdast } from "../types";

export const rehypeExpressionToMd: Plugin<[{ mdxJsxElementAllowlist?: Record<string, string[]> }?], Hast.Root> =
...
function getExpression(estree: Program) {
    return estree.body[0]?.type === "ExpressionStatement" ? estree.body[0].expression : null;
}
