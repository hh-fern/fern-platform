import { type Hast, isMdxJsxAttribute, isMdxJsxElementHast, mdxJsxAttributeToString, visit } from "@fern-docs/mdx";

export interface RehypeLinksOptions {
    replaceHref?: (href: string) => string | undefined;
}

export function rehypeLinks({ replaceHref }: RehypeLinksOptions = {}): (ast: Hast.Root) => void {
    return (ast) => {
        if (replaceHref == null) {
            return;
        }
