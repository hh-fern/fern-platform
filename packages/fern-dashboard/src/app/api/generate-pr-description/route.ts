...
export async function POST(req: NextRequest) {
  ...
  const parsedBody = await parseNextRequestBody(req, GeneratePrDescriptionRequest);
  if (parsedBody.errorResponse != null) {
    return parsedBody.errorResponse;
  }
  const { owner, repo, branch, baseBranch } = parsedBody.data;

  return NextResponse.json(await handler(userId, { owner, repo, branch, baseBranch }));
} 