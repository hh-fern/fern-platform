...
interface DomainStatusContextType {
  ...
  getOverallStatus: (domains: string[]) => 'live' | 'pending' | 'verifying' | 'error';
}
  ...
  try {
    ...
    if (stored) {
      ...
      Object.entries(parsed).forEach(([domain, status]: [string, any]) => {
        ...
        if (status?.status === 'ready' || (status?.timestamp && status.timestamp > oneDayAgo)) {
...
export function DomainStatusProvider({ children }: { children: ReactNode }) {
  ...
  const setDomainStatus = useCallback((domain: string, status: DomainStatus) => {
  ...
  const setVerifyingDomain = useCallback((domain: string, isVerifying: boolean) => {
  ...
  const getOverallStatus = useCallback((domains: string[]) => {
    if (domains.length === 0) return 'live'; // No custom domains = live

    const customDomains = domains.filter(domain => !domain.includes('buildwithfern.com'));
    if (customDomains.length === 0) return 'live'; // Only Fern subdomains = live

    // Check if any domain is being verified
    const hasVerifying = customDomains.some(domain => verifyingDomains.has(domain));
    if (hasVerifying) return 'verifying';

    // Check domain statuses
    const statuses = customDomains.map(domain => domainStatuses[domain]?.status);
  ...
  return (
    <DomainStatusContext.Provider value={{
      domainStatuses,
      setDomainStatus,
      verifyingDomains,
      setVerifyingDomain,
