...
function extractCSSFromStyleTags(jsxContent: string): {
  ...
  const cssMatches = jsxContent.match(
  ...
  const contentWithoutStyles = jsxContent.replace(
    /<style>\s*{\s*`[\s\S]*?`\s*}\s*<\/style>/g,
    ""
  );
...
function convertBooleanAttributes(content: string): string {
  return content
    ...
    .replace(/(\w+)=\{false\}/g, "")
    .replace(
      new RegExp(`\\b(${BOOLEAN_ATTRIBUTES.join("|")})\\b(?=\\s|>|\\/>)`, "g"),
      "$1"
    );
}
...
export function convertJSXToHTML(jsxContent: string): {
  ...
  const { content: contentWithoutStyles, css: extractedCSS } =
    extractCSSFromStyleTags(jsxContent);
