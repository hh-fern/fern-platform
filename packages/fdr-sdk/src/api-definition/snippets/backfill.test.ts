...
describe("backfillSnippets", () => {
    it("should backfill snippets for a search endpoint example", async () => {
        ...
        const isHttpSnippetsEnabled = true;
        const alwaysEnableJavaScriptFetch = true;

        const result = await backfillSnippets({
            apiDefinition,
            dynamicIr: undefined,
            httpSnippets: isHttpSnippetsEnabled,
    ...
    it("should backfill snippets with no http snippets", async () => {
        ...
        const isHttpSnippetsEnabled = false;
        const alwaysEnableJavaScriptFetch = true;
    ...
    it("should backfill snippets with ruby and curl http snippets", async () => {
        const apiDefinition: ApiDefinition = {
            id: ApiDefinitionId("test-api"),
            endpoints: {
                [EndpointId("search")]: {
    ...
    it("should backfill snippets with just ruby http snippets", async () => {
        ...
        expect(snippets).toBeDefined();
        const curlSnippets = snippets?.curl;
        expect(curlSnippets).toBeUndefined();
